...
if (isset($_POST['update'])){
		...
		$SQLinsert = $odb -> prepare("UPDATE `type_free` SET `raison` = ?, `lien` = ? WHERE `id` = ?");
...
if (isset($_POST['delete'])) {
    $delete = $_POST['delete'];
    $SQL = $odb -> prepare("SELECT * FROM `type_free` WHERE `id` = ?");
    $SQL -> execute(array($delete));
    $name = $SQL -> fetch(PDO::FETCH_ASSOC)['name'];
    $SQL = $odb -> prepare("DELETE FROM `type_free` WHERE `id` = ?");
    $SQL -> execute(array($delete));
    $SQL = $odb -> prepare("DELETE FROM `accounts_free` WHERE `type` = ?");
...
if (isset($_POST['addmethod'])) {
    if (empty($_POST['name'])) {
        ...
        $name = $_POST['name'];
				$img = $_POST['img'];
        $SQLinsert = $odb -> prepare("INSERT INTO `type_free` VALUES(NULL, ?, ?, ?, ?)");
...
if (isset($_POST['add'])) {
    switch ($_POST['add']) {
        case 'single':
            ...
            $pass = $_POST['pass1'];
            $type = $_POST['type1'];
            $insert = $odb -> prepare("INSERT INTO `accounts_free` VALUES(NULL, ?, ?, ?)");
      ...
      <div class="block block-rounded block-fx-pop block-themed">
        ...
        <div class="block-content block-content-full">
          <table class="table">
            ...
            <tr>
                <?php
                $SQLGetMethods = $odb -> query("SELECT * FROM `type_free`");
                                    ...
                                    <div class="block-content">
                                      <div class="block-content block block-content">
                                          ...
                                          <div class="form-group">
                                            <div class="col-sm-12">
                                              <div class="form-material">
                                              <input placeholder="Leave empty if no particular reason" class="form-control" type="text" id="lien" name="lien" value="<?php echo htmlspecialchars($lien); ?>">
              ...
              <div class="col-sm-12">
                <div class="form-material floating open">
                  ...
                  <select class="form-control" id="type" name="type1" size="1">
                    <?php
                                          $SQLGetLogs = $odb->query("SELECT * FROM `type_free` ORDER BY `id` ASC");
