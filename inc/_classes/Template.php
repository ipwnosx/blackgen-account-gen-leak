...
class Template {
    ...
    private function build_nav_array($nav_array, $nav_header, $nav_header_icons) {
        foreach ($nav_array as $node) {
            ...
            if (isset($node['target'])){$link_target = 'target="_blank"';}else{ $link_target = '';}
            ...
            if ($link_type == 'heading') {
                ...
                // Set menu properties
                if ($abo) {
                  $li_prop        = ' style="display:none" class="nav-main-item' . ($sub_active ? ' open' : '') . '"';
                }
                else {
                ...
                $this->nav_html .= "<a $link_target $link_prop href=\"$link_url\">\n$link_icon$link_name$link_badge</a>\n";
