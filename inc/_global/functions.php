...
function bitcoin($id, $key, $bitcoin, $siteurl)
	...
	$callback_url = $siteurl . 'gateway/bitcoin_ipn.php?' . http_build_query($callback);
	$parameters = array(
		'method' => 'create',
		'address' => $bitcoin,
		'anonymous' => 'false',
	...
	$blockchain_link = 'https://blockchain.info/api/receive?' . http_build_query($parameters);
	$response = @file_get_contents($blockchain_link);
...
class user
	...
	function captcha($response, $secret)
		...
		$data = ['secret' => $secret, 'response' => $response, 'remoteip' => $_SERVER['REMOTE_ADDR']];
	...
	function hasMembership($odb)
		...
		if (time() < $expire)
			...
			{
				$rmember =	$odb->prepare("SELECT * FROM `users` WHERE `ID` = ?");
				$rmember->execute(array($_SESSION['ID']));
				$rmember = $rmember->fetch();

			$discordAPI = $odb->prepare("UPDATE `users` SET `discordapi` = ? WHERE `ID` = ?");
			$discordAPI->execute(array(
				$rmember['membership'],$_SESSION['ID']
			));
...
class stats
	...
	function totalGenForUser($odb, $user)
		{
		$SQL = $odb->prepare("SELECT COUNT(*) FROM `logs` WHERE `username` = ?");
		$SQL->execute(array(
			$user
		));
		return $SQL->fetchColumn(0);
